...
class Heat_transfer:
    def __init__(self, SMAHC, dx, T_init, a_air, dt): # a_air 35
        self.name = '1D elastomer substrate'
        self.S = SMAHC
        ...
        self.dt_max = 0.5*(self.dx2/self.D_s)
        ...
        self.E_cond_const =  self.S.d.lam * self.l* self.b * self.dt_max /(self.dx * 1000) 
    ...
    def do_timestep(self, u0_d, u_d, u0_s, u_s, Tsma0, Tsma):
        ...
        for step in range(1,self.nsteps+1):
            ...
            u_d[-1] = u0_d[-1] + (self.dt_max/self.dx2) * self.D_d * (u0_d[-2] - u0_d[-1]) + (self.dt_max/self.dx2) * self.D_d * (u0_s[0] - u0_d[-1])
            
            
            u_s[0] = u0_s[0] + (self.dt_max/self.dx2) * self.D_s * (u0_s[1] - u0_s[0]) + (self.dt_max/self.dx2) * self.D_d * (u0_d[-1] - u0_s[0])
            #u_s[1] = self.T_init
            u_s[1:-1] = u0_s[1:-1] + (self.dt_max/self.dx2) * self.D_s * (u0_s[2:]  + u0_s[:-2]  - 2*u0_s[1:-1])
            u_s[-1] = u0_s[-1] + (self.dt_max/self.dx2) * self.D_s * (u0_s[-2] - u0_s[-1]) + self.C_s*self.dt_max*(self.T_init - u0_s[-1])
